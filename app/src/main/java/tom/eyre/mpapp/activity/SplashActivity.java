...
import tom.eyre.mpapp.service.UpdateService;
...
//                try {
//                    updateService.updateDivision(databaseUtil.mpDB);
...
//        });
        futureTwo = executorService.submit(new Callable<Boolean>() {
            ...
            public Boolean call() throws Exception {
                try {
                    updateService.updateMp(databaseUtil.mpDB);
                } catch (JsonProcessingException e) {
                    e.printStackTrace();
                } catch (JSONException e) {
                    e.printStackTrace();
        ...
        futureThree = executorService.submit(new Callable<Boolean>() {
            @Override
            public Boolean call() throws Exception {
                updateService.updateBills(databaseUtil.mpDB);
                return true;
            }
        });
    ...
    private void getVotes(){
        ...
        future = executorService.submit(new Callable<Boolean>() {
            ...
            public Boolean call() throws Exception {
                try {
                    updateService.updateVotes(databaseUtil.mpDB,getApplicationContext());
                } catch (JsonProcessingException e) {
                    e.printStackTrace();
